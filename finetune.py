...
class CFG:
    ...
    outputs_path = f'./{output_save_folder}/{current_day}/{str(int(time.time()))}/{exp_name}/{args.valid_id}/'
...
def read_luke_image(fragment_id):
    ...
    for i in tqdm(idxs):
        image = cv2.imread(f"promising_cropped_7_2/surface_volume/{i:02}.tif", 0)
    ...
    inklabels = cv2.imread("promising_cropped_7_2/inital_mask.png")
    inklabels = zoom(inklabels, 2, mode='nearest', order=0)[:,:,0]
    inklabels = (inklabels.astype(np.float16) / 255).astype(np.int32)

    #images, ink labels, mask
    return images, inklabels, mask
...
def get_train_valid_dataset():
    ...
    for fragment_id in range(1):
        ...
        if fragment_id == 2:
            image, mask, mask_frag = read_luke_image(2)
            ...
            image, mask, mask_frag = read_luke_image(fragment_id)
...
for epoch in range(15):
    ...
    avg_loss = 0
    # train, skip first epoch so you get eval on first model
    if epoch != 0:
    ...
    Logger.info(
        f'Epoch {epoch+1} - avg_train_loss: {avg_loss:.4f}  avg_val_loss: {avg_val_loss:.4f}  time: {elapsed:.0f}s LR: {CFG.lr}')
    ...
    torch.save({'model': model.state_dict()},
                CFG.model_dir + f'FINETUNED_{CFG.model_name}_fold{fold}_epoch{epoch+1}_score{score}.pth')
