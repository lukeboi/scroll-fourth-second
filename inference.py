...
# weights_path_resnet152 = 'checkpoints/2023_07_03/2023_07_03_256_22_batchsize16_diceloss0_tverskyloss0_stride3_cropping_num12-22_ls0.3_clip50_200/1/vesuvius-models/try'
weights_path_resnet152 = '/home/ubuntu/Competition/kaggle/checkpoints/2023_07_05/1688527617/2023_07_05_256_22_batchsize16_diceloss0_tverskyloss0_stride3_cropping_num12-22_ls0.3_clip50_200/1/vesuvius-models/try'
...
def read_luke_image(fragment_id):
    images = []
    masks = []

    global sample_image
    global start_idx_layer

    # idxs = range(65)
    mid = 65 // 2
    # start = mid - CFG.in_chans // 2
    # end = mid + CFG.in_chans // 2
    start = start_idx_layer
    end = start_idx_layer + 22
    ...
    for i in tqdm(idxs):
        image = cv2.imread(f"promising_fragment/{i:02}.tif", 0)
        image = np.rot90(image, k=-1, axes=(0, 1))

        # crop
        # image = image[850:850+3072, 0:4096]
        iamge = image[850:850+3072, 0:6400]
        images.append(image)
    ...
    print("zooming")
    images = zoom(images, 2, mode='nearest', order=0)[:,:,11:33]
    # images = np.resize(images, np.array(images.shape) * 2)[:,:,11:33]
    # cv2.resize(images, np.array(images.shape) * 2, interpolation=cv2.INTER_CUBIC)[:,:,11:33]

    print("masking")
    mask = np.sum(images, axis=2) > 0

    # write mask for debugging
    cv2.imwrite("out.png", (np.sum(images, axis=2) > 0).astype(np.uint8) * 255)
...
def cdf(hist):
    """Compute the cumulative distribution function for a histogram."""
    cdf = hist.cumsum()
    cdf = cdf / cdf[-1]  # Normalize
    return cdf

def hist_matching(src, template):
    """Adjust the pixel values of a grayscale image such that its histogram
    matches that of a target image"""

    src_hist, src_bins = np.histogram(src.flatten(), 256, [0,256])
    template_hist, template_bins = np.histogram(template.flatten(), 256, [0,256])

    src_cdf = cdf(src_hist)
    template_cdf = cdf(template_hist)

    # create a mapping s.t. src_cdf(x) = template_cdf(new_pixel_value)
    # then replace each pixel with new_pixel_value
    inverse_cdf = interp1d(src_cdf, src_bins[:-1])
    new_pixel_values = np.interp(template_cdf, src_cdf, src_bins[:-1])

    # Print out mapping
    for i, new_val in enumerate(new_pixel_values):
        print(f"Pixel value {src_bins[i]} maps to {new_val}")

    src_matched = inverse_cdf(src.ravel()).reshape(src.shape)

    return src_matched

def make_test_dataset(fragment_id):
    test_images, mask = read_luke_image(fragment_id)
    test_images_frag, mask_frag = read_image('b')

    # test_images = hist_matching(test_images, test_images_frag)

    # Get histogram of images
    if False:
        # Replace all zero values with NaN
        data = np.where(test_images==0, np.nan, test_images)

        # Flatten the 3D array into 1D and remove all NaN values
        data_flat = data.flatten()
        data_no_zero = data_flat[~np.isnan(data_flat)]

        mu, std = norm.fit(data_no_zero)

        # Plot histogram
        n, bins, patches = plt.hist(data_no_zero, bins=255)

        # Compute and plot the CDF
        a = cumfreq(data_no_zero, numbins=20)
        x = np.linspace(0, 255, a.cumcount.size)
        y = a.cumcount / max(a.cumcount) * np.max(n)  # scale CDF to fit the histogram
        ...
        p = norm.pdf(x, mu, std)
        p = p * np.max(n) / np.max(p)
        plt.plot(x, p, 'k', linewidth=2)
        plt.title('Histogram of 3D Array Values (Excluding Zero)')
        plt.savefig("foo.png")
        plt.show()
...
os.makedirs(f"inference/{MODEL_TIME}/")

results = []
# for fragment_id in fragment_ids:
for i in range(16, 38, 2):
# for i in [14, 16, 18, 19, 20, 21, 22, 23, 24, 25, 26, 28, 30]:
    start_idx_layer = i

    test_loader, xyxys = make_test_dataset('a')

    # binary_mask = cv2.imread(CFG.comp_dataset_path + f"{mode}/{fragment_id}/mask.png", 0)
    # binary_mask = cv2.imread("./promising_Fragment/34.tif", 0)
    binary_mask = (cv2.resize(sample_image.T, np.array(sample_image.T.shape) * 2) > 0).astype(int)
    # binary_mask = (cv2.resize(binary_mask, np.array(binary_mask.shape) * 2, interpolation=cv2.INTER_CUBIC) / 255).astype(int)
    ...
    for threshold in [0.3, 0.4, 0.5, 0.6, 0.7]:
        if remove_small_object:
            pred = remove_small_objects(mask_pred >= threshold, remove_small_objects_degree)
        else:
            pred = remove_small_objects(mask_pred >= threshold, 1)

            pred = pred.astype(int)
            # pred *= binary_mask

            cv2.imwrite(os.path.join(f"inference/{MODEL_TIME}", f'pred_raw_start{start_idx_layer}_scaled2.png'), mask_pred * 255)
            cv2.imwrite(os.path.join(f"inference/{MODEL_TIME}", f'pred_superimpose_thresh{threshold}_start{start_idx_layer}_scaled2.png'), pred * 210 + cv2.resize(sample_image.T, np.array(sample_image.T.shape) * 2) / 5)
